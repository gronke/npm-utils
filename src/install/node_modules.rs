...
use crate::registry::{Registry, ResolveEvent, Resolved};
...
pub fn node_modules(
    ...
    node_modules_observed(package_json, dest, |_| {}, |_| {})
}

/// [`node_modules`] with progress observers: the resolve walk's [`ResolveEvent`]s, then one
/// [`InstallEvent::Fetch`](super::InstallEvent::Fetch) immediately before each package's
/// download/verify/extract. A skip-if-unchanged cache hit runs no populate step and emits no
/// install events.
pub(crate) fn node_modules_observed(
    package_json: &Path,
    dest: &Path,
    on_resolve: impl Fn(ResolveEvent<'_>) + Sync,
    mut on_install: impl FnMut(super::InstallEvent<'_>),
) -> Result<Vec<Resolved>, Box<dyn std::error::Error + Send + Sync>> {
    let roots = root_requirements(package_json)?;
    let resolved = Registry::npm().resolve_tree_observed(&roots, on_resolve)?;
    let want = resolved
        .iter()
    ...
    super::run_install(dest, &want, |node_modules| {
        for (i, pkg) in resolved.iter().enumerate() {
            ...
            on_install(super::InstallEvent::Fetch {
                index: i + 1,
                total: resolved.len(),
                name: &pkg.name,
                version: &version,
...
fn root_requirements(
    package_json: &Path,
) -> Result<Vec<(String, Range)>, Box<dyn std::error::Error + Send + Sync>> {
